...
Each model will be paired with a visualization then a summary of the specific values tested afterwards.
...
$$
```{r echo=FALSE}
plot(x = df$edu_exp, y = df$female_labor_rate, main = "Female Labor Rate vs Education Expenditure", xlab = "Education Expenditure", ylab = "Female Labor Rate",
)
...
```{r echo=FALSE}
avPlots(model1, col.lines = "red", id = FALSE, main = "Education Exp. with Control Variables")
```
```{r echo = FALSE}
summary(model1)
```
...
$$
```{r echo=FALSE}
avPlots(model2, col.lines = "red", id = FALSE, main = "Education Expediture with Log")
```
```{r echo = FALSE}
summary(model2)
```
...
```{r echo=FALSE}
avPlots(model3, col.lines = "red", id = FALSE, main = "Multivariate Model")
```
```{r echo = FALSE}
summary(model3)
```
...
$$
```{r echo=FALSE}
avPlots(fertxlabor1, col.lines = "red", id = FALSE, main = "Female Labor Rate vs. Fertility Rate")
```
```{r echo = FALSE}
summary(fertxlabor1)
```
...
$$
```{r echo=FALSE}
avPlots(fertxlabor2, col.lines = "red", id = FALSE, main = "Quadratic Representation")
```
```{r echo = FALSE}
summary(fertxlabor2)
```
...
```{r echo=FALSE}
avPlots(model4, col.lines = "red", id = FALSE, main = "Multivariate Model with Quadratic")
```
```{r echo = FALSE}
summary(model4)
```
